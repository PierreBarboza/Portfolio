  ...
  const avatarConfig = event => {
    if (event.currentTarget.id === 'save') {
      if (countSelected !== 0) {
        ...
        props.setAtualizarAvatar(idAvatar);
      }
    }
    ...
    <div>
      <button className="btn_avatar" onClick={abrirModal}>
        ...
        <div id="avatarSelection" className="select_avatar">
          <img id="avatarPage" src={avatarDefault} className="avatarPage" style={{ width: "80px", borderRadius: "45px" }} />
          <span>
      ...
      >
        ...
        <div className='footer_modal'>
          <button id='save' onClick={avatarConfig} style={{ display: "none" }}>Salvar</button>
        </div>
      </Modal>
    </div>
  );
